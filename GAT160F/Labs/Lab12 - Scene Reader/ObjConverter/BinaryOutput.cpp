...
int BinaryOutput::WriteHeader(std::ofstream & out, int numVerts, int numIndices, int sizeVerts, int sizeIndices, int format)
{
	...
	totalBytes += WriteInt(out, sizeof(GLenum));
	totalBytes += WriteInt(out, sizeof(GLuint));
	totalBytes += WriteInt(out, sizeof(GLuint));
	totalBytes += WriteInt(out, sizeof(GLuint));
...
int BinaryOutput::WriteVertexData(std::ofstream & out, int numVerts, glm::vec3 * verts, glm::vec3 * colors, glm::vec2* textures, glm::vec3 * normals, Index* indices, int format)
{
	...
	{
		if (format & HasPosition)
			totalBytes += WriteVec3(out, verts[indices[j].indicies[positionVari]-1]);
		if (format & HasColor)
			totalBytes += WriteVec3(out, colors[indices[j].indicies[colorVari]-1]);
		if (format & HasTexture)
			totalBytes += WriteVec2(out, textures[indices[j].indicies[textureVari]-1]);
		if (format & HasNormal)
			totalBytes += WriteVec3(out, normals[indices[j].indicies[normalVari]-1]);
	}
	return totalBytes;
}
