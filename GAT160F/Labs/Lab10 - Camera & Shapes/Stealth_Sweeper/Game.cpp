...
bool Game::Initialize(MeGlWindow* window)
{
	...
	cube =  new Renderable(m_window->width(), m_window->height(),&programID,generator->MakeCube(), m_logs);
	cube2 = new Renderable(m_window->width(), m_window->height(),&programID,generator->MakeCube(), m_logs);
...
void Game::BufferBind()
{
	...
	for (int I = 0; I < generator->usedGeometries; I++)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, generator->shapes[I].numbVerts*sizeof(Vector), generator->shapes[I].verts, GL_STATIC_DRAW);
...
bool Game::Update(float dt)
{
	...
	if (!m_paused)
	{
		cube->Update(glm::vec3(0.0f, 0.0f, -3.0f), 40.0f);
		cube2->Update(glm::vec3(5.0f, 0.0f, -3.0f),40.0f);
	}
