...
bool Game::Initialize(MeGlWindow* window)
{
	...
	cube   .setLogger(m_logs).setShaderInfo(&shaders[0]).setShape(generator->MakeObject("../Data/Scenes/AIWorld.pct.scene")).setTexture("../Data/Textures/world_texture.bmp");
	dragon .setLogger(m_logs).setShaderInfo(&shaders[1]).setShape(generator->MakeObject("../Data/Scenes/Dargon.pc.scene"));
	dragon2.setLogger(m_logs).setShaderInfo(&shaders[1]).setShape(generator->MakeObject("../Data/Scenes/Dargon.pc.scene"));
	special.setLogger(m_logs).setShaderInfo(&shaders[2]).setShape(generator->MakeObject("../Data/Scenes/SpecialCube.pt.scene")).setTexture("../Data/Textures/browns.bmp");
	...
	m_renderEngine.addRenderable(&special);
