...
bool Game::Initialize(MeGlWindow* window)
{
	...
	cube = new Cube(m_window->width(), m_window->height(),&programID, generator.getCube());
	BufferBind();
	installShaders();
...
void Game::BufferBind()
{
	...
	glBufferData(GL_ARRAY_BUFFER, cube->data.numbVerts, cube->data.verts, GL_STATIC_DRAW);

	GLuint indexBufferID;
	glGenBuffers(1, &indexBufferID);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferID);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, cube->data.numbInd, cube->data.indicies, GL_STATIC_DRAW);
