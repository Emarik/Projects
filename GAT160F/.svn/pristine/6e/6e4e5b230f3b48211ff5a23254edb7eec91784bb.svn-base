...
#include <string.h>
#include "sstream"
#define MAXVERTEXES 100000
#define MAXINDEXES  100000
int maxVerts = 0;
int maxIndex = 0;
float randomFloat()
{
...
void ReadObj::ReadIndex(std::string str)
{
	...
	else if (second == first + 1)
	{
		...
		Indexes[IndexesUsed].indicies[normalVari] = m_config->getIntFromString(str.substr(second));
	}
	else if (second == -1)
	{
		Indexes[IndexesUsed].indicies[positionVari] = m_config->getIntFromString(str.substr(0, first));
		Indexes[IndexesUsed].indicies[textureVari] = m_config->getIntFromString(str.substr(first));
	}
	else
	{
		Indexes[IndexesUsed].indicies[positionVari] = m_config->getIntFromString(str.substr(0, first));
		Indexes[IndexesUsed].indicies[textureVari] = m_config->getIntFromString(str.substr(first,second-first));
		Indexes[IndexesUsed].indicies[normalVari] = m_config->getIntFromString(str.substr(second));
	}
	IndexesUsed++;
}
